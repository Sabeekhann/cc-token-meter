...
| --- | --- | --- |
| CI / Required CI | PRs, pushes to `main`, manual | Ubuntu/Node 24 required gate: project policy, unit/contract tests, production audit, package dry-run, CLI smoke tests, and conflict-marker detection |
...
| Compatibility | Pushes to `main`, manual | Node 20/22/26 on Linux and Node 24 on macOS/Windows; unit tests plus packed-artifact lifecycle, with large-history budgets on Node 20/24/26 |
...
2. Push corrections while Required CI, PR Governance, informational Coverage,
   informational Dependency Review, and Corgea provide feedback.
3. Complete the PR template and self-review before marking the PR ready.
4. Wait for both required statuses to pass:
...
`.github/CODEOWNERS` records repository ownership, but the current solo-
maintainer ruleset does not require Code Owner approval. The active **Protect
main** ruleset targets the default branch and enforces:
...
The zero-review setting avoids a self-review deadlock for a personal repository
where the maintainer is also the PR author and sole Code Owner. Human review can
still be requested when useful, but it is not a merge-rule requirement.
...
and Compatibility run outside the pull-request path, so contributor PRs do not
wait on macOS, Windows, CodeQL, or full-history gitleaks work.
...
policy, package contents, the production dependency footprint, command entry
points, and committed conflict markers in one result. Coverage and dependency
diff review provide additional advisory signals without making the required
path fragile. Slower platform and security jobs still run on every update to
`main` and can be started manually before a high-risk release.
